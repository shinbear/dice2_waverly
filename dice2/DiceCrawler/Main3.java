...
public class Main3 {
	...
	public static int initSearch(String unedname, String engine) throws IOException {
		...
		if (engine.equals("bingSearch")) {
			...
			URL = "https://www.google.com/search?q=" + unedname + "&num=10";
	...
	public static void getCV(String URL) throws IOException, SQLException {
		if (URL.substring((URL.length() - 4), URL.length()).equals(".pdf")) {
			...
			createDir(dirName);
			String res = downloadFromUrl(URL, dirName);
			...
			try {
				...
				for (Element link : links) {
					String regEx = "(download\\s*CV)|(download\\s*C.V.)|resume|CV|(View/Download C.V.)";
					// 编译正则表达式-忽略大小写的写法
					...
					while (matcher.find()) {
						...
						String res = downloadFromUrl(linkHref, dirName);
						System.out.println(res);
					}
				}
			} catch (Exception e) {
		...
		try {
			...
			if (result.contains("Sign Out")) {
				...
				if (background_text != null) {
					...
					// iMatch method to check if including sensitive content
					boolean iMatch = iMatch(background_text.toString());
					if (iMatch) {
						writefile(filePath(), background_text.toString(), false);
						Sub_ID++;
					}
	...
	public static String downloadFromUrl(String url, String dir) {
		try {
			...
			File f = new File(dir + "_" + fileName);
	...
	public static boolean iMatch(String str) {
	...
	public static String processDLURL(String url, String linkHref) {
		...
		if (linkHref.substring(0, 4) == "http") {
			return linkHref;
		} else if (linkHref.toString().substring(0, 1) == "/") {
